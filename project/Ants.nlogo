...
to setup-obstacle
  ...
  ;; create single obstacle in the center
  create-obstacle 0 0 4 16
...
to setup-obstacles
  ...
  ;; create two walls
  create-obstacle 0 (max-pycor - 8) 4 16
  create-obstacle 0 (min-pycor + 8) 4 16
...
to setup-nest  ;; patch procedure
  ...
  set nest? (distancexy (-0.8 * max-pxcor) 0) < 5
  ;; spread a nest-scent over the whole world -- stronger near the nest
  set nest-scent 200 - distancexy (-0.8 * max-pxcor) 0
end

to setup-food  ;; patch procedure
  ;; setup food source one (in alto)
...
to look-for-food  ;; turtle procedure
  ...
  [
    if sensing-timer >= 50 and pheromone-food > 0.1[
      ;; reset sensing timer
      set sensing-timer 0
...
to create-obstacle [center-x center-y size-x size-y]
  let half-size-x (size-x / 2)
  let half-size-y (size-y / 2)
  ask patches with [
    pxcor >= (center-x - half-size-x) and pxcor <= (center-x + half-size-x) and
    pycor >= (center-y - half-size-y) and pycor <= (center-y + half-size-y)
  ] [
    set obstacle? true
    set pcolor yellow
  ]
...
"" ""
PENS
"food-in-pile-1" 1.0 0 -11221820 true "" "plot sum [food] of patches with [food-source-number = 1]"
"food-in-pile-2" 1.0 0 -14439633 true "" "plot sum [food] of patches with [food-source-number = 2]"
